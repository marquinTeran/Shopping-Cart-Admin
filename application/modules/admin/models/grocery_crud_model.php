...
class grocery_CRUD_Model  extends CI_Model  {
    ...
    function db_update($post_array, $primary_key_value)
    {
    	$primary_key_field = $this->get_primary_key();
			$data = array(
			   'CODIGO_USUARIO' => $this->session->userdata('usuario') ,
			   'FECHA' => date("Y-m-d H:i:s") ,
			   'ACCION' => $this->table_name.'-MODIFICO',
			   'ID_AFECTADO' => "$primary_key_value"
			);
    ...
    print_r($post_array);
    	...
    	if($insert)
    	{
			...
			$data = array(
			   'CODIGO_USUARIO' => $this->session->userdata('usuario') ,
			   'FECHA' => date("Y-m-d H:i:s") ,
			   'ACCION' => $this->table_name.'-INGRESO',
			   'ID_AFECTADO' => $id
			);
			$this->Api->insert_auditoria($data);
    		return $id;
    ...
    function db_delete($primary_key_value)
    {
    	...
    	if( $this->db->affected_rows() != 1){    	
		    ...
		    $data = array(
		       'CODIGO_USUARIO' => $this->session->userdata('usuario') ,
		       'FECHA' => date("Y-m-d H:i:s") ,
		       'ACCION' => $this->table_name.'-ELIMINAR',
		       'ID_AFECTADO' => $primary_key_value
		    );
				$this->Api->insert_auditoria($data);    	
    		return true;
